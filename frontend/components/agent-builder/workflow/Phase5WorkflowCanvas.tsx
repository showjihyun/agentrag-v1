...
interface Phase5WorkflowCanvasProps {
  workflowId?: string;
  initialNodes?: Node[];
  initialEdges?: Edge[];
...
function Phase5WorkflowCanvasInner({
  ...
  const [activeTab, setActiveTab] = useState<'canvas' | 'debug' | 'performance' | 'ai'>('canvas');
            ...
            <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="flex-1 flex flex-col">
              ...
              <TabsContent value="ai" className="flex-1 m-0 p-4">
                <AIAssistantPanel
                  ...
                  onApplyBreakpoint={handleApplyBreakpoint}
                />
              </TabsContent>
            </Tabs>
          </div>
...
export function Phase5WorkflowCanvas(props: Phase5WorkflowCanvasProps) {
  return (
    <ReactFlowProvider>
      <Phase5WorkflowCanvasInner {...props} />
    </ReactFlowProvider>
  );
}
